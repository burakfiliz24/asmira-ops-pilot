...
export default function Sidebar() {
  ...
  const { isSyncing, isOnline, lastSyncTime } = useSyncStatusStore();
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-72 shrink-0 flex-col bg-[#0b1120]/80 text-white shadow-[4px_0_24px_-2px_rgba(0,0,0,0.5)] backdrop-blur-md lg:flex">
      {/* Top: Logo */}
      <div className="relative flex w-full flex-col items-center justify-center px-4 pb-1 pt-5">
        <Image
        ...
        <div className="relative mt-2 h-20 w-full overflow-hidden">
          ...
          {/* Stars */}
          <div className="absolute left-[15%] top-[10%] h-[2px] w-[2px] rounded-full bg-white/30" />
          <div className="absolute left-[45%] top-[6%] h-[1.5px] w-[1.5px] rounded-full bg-white/20" />
          <div className="absolute left-[75%] top-[12%] h-[1px] w-[1px] rounded-full bg-white/25" />
          <div className="absolute left-[60%] top-[3%] h-[1.5px] w-[1.5px] rounded-full bg-white/15" />

          {/* Ship 1 - Large cargo ship */}
          <div className="ship-bob absolute bottom-[8px]">
            <div className="relative">
              <svg width="72" height="42" viewBox="0 0 48 28" fill="none">
                ...
                {/* Windows - flickering */}
                <rect className="ship-window" x="16" y="12" width="2.5" height="1.5" rx="0.3" fill="rgba(251,191,36,0.5)" />
                <rect className="ship-window-2" x="20" y="12" width="2.5" height="1.5" rx="0.3" fill="rgba(251,191,36,0.4)" />
                {/* Funnel */}
              ...
              <div className="ship-smoke absolute" style={{ left: "42px", top: "10px", width: "4px", height: "4px", borderRadius: "50%", background: "rgba(148,163,184,0.3)" }} />
              <div className="ship-smoke-2 absolute" style={{ left: "44px", top: "11px", width: "3px", height: "3px", borderRadius: "50%", background: "rgba(148,163,184,0.2)" }} />
              <div className="ship-smoke-3 absolute" style={{ left: "43px", top: "9px", width: "3.5px", height: "3.5px", borderRadius: "50%", background: "rgba(148,163,184,0.25)" }} />
              {/* Wake trail behind ship */}
              <div className="ship-wake absolute -left-[10px] bottom-[2px] h-[2px] w-[14px] rounded-full bg-gradient-to-l from-cyan-400/20 to-transparent" />
              {/* Water glow */}
              <div className="absolute -bottom-[4px] left-[10%] right-[10%] h-[5px] rounded-full bg-blue-400/15 blur-[4px]" />
            </div>
          </div>

          {/* Ship 2 - Smaller vessel, sails opposite */}
          <div className="ship-bob-2 absolute bottom-[10px]">
            <div className="relative">
              <svg width="52" height="32" viewBox="0 0 32 20" fill="none" style={{ transform: "scaleX(-1)" }}>
                ...
                {/* Window - flickering */}
                <rect className="ship-window" x="12" y="8.5" width="2" height="1.5" rx="0.3" fill="rgba(251,191,36,0.4)" />
                <rect className="ship-window-2" x="15" y="8.5" width="2" height="1.5" rx="0.3" fill="rgba(251,191,36,0.3)" />
                {/* Funnel small */}
                <rect x="20" y="9" width="2" height="3" rx="0.2" fill="rgba(6,182,212,0.1)" stroke="rgba(6,182,212,0.25)" strokeWidth="0.4" />
                {/* Mast */}
                <line x1="14" y1="3" x2="14" y2="7" stroke="rgba(148,163,184,0.25)" strokeWidth="0.4" />
                <circle cx="14" cy="3" r="0.7" fill="rgba(52,211,153,0.4)" />
              </svg>
              {/* Smoke puffs */}
              <div className="ship-smoke absolute" style={{ left: "8px", top: "6px", width: "3px", height: "3px", borderRadius: "50%", background: "rgba(148,163,184,0.25)" }} />
              <div className="ship-smoke-2 absolute" style={{ left: "9px", top: "7px", width: "2.5px", height: "2.5px", borderRadius: "50%", background: "rgba(148,163,184,0.18)" }} />
              {/* Wake trail */}
              <div className="ship-wake absolute -right-[8px] bottom-[2px] h-[1.5px] w-[10px] rounded-full bg-gradient-to-r from-cyan-400/15 to-transparent" />
              {/* Water glow */}
              <div className="absolute -bottom-[3px] left-[10%] right-[10%] h-[4px] rounded-full bg-cyan-400/10 blur-[3px]" />
            </div>
          </div>
      ...
      {/* Navigation */}
      <nav className="flex flex-1 flex-col overflow-y-auto px-3 pb-24 pt-2">
      ...
      {/* User Section */}
      <div className="absolute bottom-0 left-0 right-0 border-t border-white/[0.06] bg-[#0b1120]/95 px-3 py-2.5 backdrop-blur-md">
        <div className="flex items-center gap-2.5">
          <div className="group relative">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500/20 to-cyan-500/20 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]">
              <div className="absolute inset-0 rounded-xl bg-blue-400/0 transition-all duration-300 group-hover:bg-blue-400/10" />
              <User className="relative h-4 w-4 text-blue-400" />
            </div>
            <div className={cn(
              "absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-[#0b1120]",
              isOnline 
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5">
              <RefreshCw className={cn(
                ...
                isSyncing ? "animate-spin text-cyan-400" : "text-white/40"
              )} />
          ...
          <button
            ...
            className="group/logout flex h-7 w-7 items-center justify-center rounded-lg border border-white/[0.06] bg-white/[0.02] text-white/30 transition-all duration-300 hover:border-red-500/30 hover:bg-red-500/10 hover:text-red-400"
            title="Çıkış Yap"
          >
            <LogOut className="h-3 w-3 transition-transform duration-300 group-hover/logout:translate-x-0.5" />
          </button>
        </div>
      </div>
    </aside>
