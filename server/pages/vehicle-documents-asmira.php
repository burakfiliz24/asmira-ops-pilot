...
<div class="px-2 py-4 lg:px-0">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-white">Asmira Özmal</h1>
                <p class="text-sm text-white/50 mt-1">Asmira'ya ait çekici ve dorse evrakları</p>
            </div>
        </div>
    </div>

    <!-- Tabs: Çekiciler / Dorseler / Araç Setleri -->
    <div class="flex gap-1 mb-6 border-b border-white/10">
        <button onclick="switchTab('trucks')" class="tab-btn active px-4 py-2.5 text-sm font-medium transition-colors" id="tab-trucks">Çekiciler</button>
        <button onclick="switchTab('trailers')" class="tab-btn px-4 py-2.5 text-sm font-medium transition-colors" id="tab-trailers">Dorseler</button>
        <button onclick="switchTab('sets')" class="tab-btn px-4 py-2.5 text-sm font-medium transition-colors" id="tab-sets">Araç Setleri</button>
    </div>
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        trucks = trucks.filter(t => t.category === 'asmira');
        trailers = trailers.filter(t => t.category === 'asmira');
        vehicleSets = vehicleSets.filter(s => s.category === 'asmira');
        renderAll();
    } catch (e) {
...
function switchTab(tab) {
    document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active', 'text-blue-400', 'border-b-2', 'border-blue-400'));
    document.querySelectorAll('[id^="content-"]').forEach(c => c.classList.add('hidden'));
    const btn = document.getElementById('tab-' + tab);
    btn.classList.add('active', 'text-blue-400', 'border-b-2', 'border-blue-400');
...
function renderVehicleList(containerId, items, type, label) {
    ...
    if (items.length === 0) {
        container.innerHTML = `<div class="text-center py-12 text-white/40"><i data-lucide="truck" class="h-12 w-12 mx-auto mb-3 opacity-30"></i><p>Henüz ${label.toLowerCase()} eklenmemiş</p></div>`;
            ...
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-500/10">
                        <i data-lucide="${type === 'truck' ? 'truck' : 'container'}" class="h-5 w-5 text-blue-400"></i>
                    </div>
                    <div>
...
function renderSetList() {
    ...
    if (vehicleSets.length === 0) {
        container.innerHTML = '<div class="text-center py-12 text-white/40"><p>Henüz araç seti oluşturulmamış</p></div>';
        return;
    }
        ...
        html += `<div class="card p-4 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-500/10">
                    <i data-lucide="link" class="h-5 w-5 text-cyan-400"></i>
                </div>
                <div>
                    <span class="font-semibold text-white">${escapeHtml(truck?.plate || '?')}</span>
                    <span class="text-white/30 mx-2">+</span>
                    <span class="font-semibold text-white">${escapeHtml(trailer?.plate || '?')}</span>
                </div>
            </div>
            <button onclick="deleteSet('${s.id}')" class="h-8 w-8 flex items-center justify-center rounded-lg text-white/30 hover:text-red-400 hover:bg-red-500/10 transition">
                <i data-lucide="trash-2" class="h-4 w-4"></i>
            </button>
        </div>`;
    });
...
async function uploadDoc(input, ownerId, ownerType, docType) {
    ...
    try {
        ...
        if (ownerType === 'truck') { trucks = data.filter(t => t.category === 'asmira'); }
        else { trailers = data.filter(t => t.category === 'asmira'); }
        renderAll();
    } catch (e) {
...
async function deleteVehicle(id, type) {
    ...
    try {
        ...
        await apiRequest(endpoint, { method: 'DELETE', body: JSON.stringify({ id }) });
